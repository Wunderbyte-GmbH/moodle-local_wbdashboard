...
$string['settings:lockedfilters_desc'] = 'Lock filter keys to user profile fields, one mapping per line as "filterkey=profilefieldshortname" (e.g. "region=region"). For every user without the "local/wb_dashboard:ignorelockedfilters" capability, a mapped filter key is forced server-side to that user\'s own profile field value on all charts and digits; the [chartfilter] control for the key is replaced by a static value. The profile field value must exactly match the report filter\'s option values (case and spelling). A locked user with an empty profile field sees no data.';
